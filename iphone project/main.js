$(function(){
    ...
    const globalState = {
      apps: [
        ...
        {
          nombre: 'watch',
          icono: 'https://firebasestorage.googleapis.com/v0/b/fotos-3cba1.appspot.com/o/ios14%2Fwatch.png?alt=media&token=d0d76558-79c6-4d21-8fe7-1d13de3a6a3e',
          type: 'app',
          dinamico: true
        },
        {
      ...
      dateTime: {
        meses: ['January', 'Febuary', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'],
        dias: ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thrusday', 'Friday', 'Saturday']
      },
      bateriaBaja: false,
      draggScreen: false
    }
  ...
  <path d="M62 38l-7.259-9.86L46 36"></path>
    ...
    function renderizarUI(){
      ...
      //Si existe el widget del Calender
      if ($('.wrapperApps .app[data-app="widgetFullCalender"]').length) {
        //Preparamos el widget del Calender
        $('.wrapperApps .app[data-app="widgetFullCalender"] .icono').append('<div class="eventos"><p>No more events for today</p></div><div class="CalenderWrapper"></div>');
        //Creamos el Calender del widget
        $('.wrapperApps .app[data-app="widgetFullCalender"] .icono .CalenderWrapper').Calender();
      }
      //Si existe el icono dinamico del Calender
      if ($('.wrapperApps .app.CalenderDinamico').length) {
        //Icono dinamico del Calender
        $('.wrapperApps .app.CalenderDinamico .icono').fechaIcono();
      }
      //Si existe el watch analogico dinamico
      if ($('.wrapperApps .app.watchDinamico').length) {
        //watch analogico dinamico
        $('.wrapperApps .app.watchDinamico .icono').watch();
      }
    }
    function encendido(){
      renderizarUI();
